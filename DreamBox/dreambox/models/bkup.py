        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = self.loss_bbox(
                    ...
                    reduction_override=reduction_override)
                # Perform OOD only for FG objects
                if ood_score is not None:
                    if ood_score.numel() > 0:
                        loss_ood_ = self.loss_ood(
                            ood_score[pos_inds.type(torch.bool)],
                            ood_labels[pos_inds.type(torch.bool)].to(torch.int),
                            reduction_override=reduction_override)
                        ...
                        if self.custom_activation:
                            acc_ = self.loss_ood.get_accuracy(ood_score[pos_inds.type(torch.bool)],
                                                              ood_labels[pos_inds.type(torch.bool)])
                            losses.update(acc_)
                        else:
                            ood_preds = torch.stack([1 - ood_score[pos_inds.type(torch.bool)].sigmoid(),
                                                     ood_score[pos_inds.type(torch.bool)].sigmoid()], dim=1)
                            losses['ood_acc'] = accuracy(ood_preds, ood_labels[pos_inds.type(torch.bool)])
            else:
