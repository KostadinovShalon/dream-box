...
class MaxIoUAssignerWithOOD(MaxIoUAssigner):
    ...
    def assign(self,
        ...
        gt_ood_labels = gt_instances.ood_labels
        ...
        if assign_on_cpu:
            ...
            gt_ood_labels = gt_ood_labels.cpu()
        ...
        assign_result = self.assign_wrt_overlaps(overlaps, gt_labels, gt_ood_labels)
        if assign_on_cpu:
            assign_result.gt_inds = assign_result.gt_inds.to(device)
            assign_result.max_overlaps = assign_result.max_overlaps.to(device)
            if assign_result.labels is not None:
                assign_result.labels = assign_result.labels.to(device)
            if assign_result.ood_labels is not None:
                assign_result.ood_labels = assign_result.ood_labels.to(device)
        return assign_result

    def assign_wrt_overlaps(self, overlaps: Tensor,
                            gt_labels: Tensor, gt_ood_labels: Tensor) -> AssignResultWithOOD:
        ...
        if num_gts == 0 or num_bboxes == 0:
            ...
            assigned_labels = overlaps.new_full((num_bboxes, ),
                                                -1,
                                                dtype=torch.long)
            assigned_ood_labels = overlaps.new_full((num_bboxes, ),
                                                0,
                                                dtype=torch.long)
            if num_gts == 0:
                # No truth, assign everything to background
                assigned_gt_inds[:] = 0
            return AssignResultWithOOD(
                num_gts=num_gts,
                gt_inds=assigned_gt_inds,
                max_overlaps=max_overlaps,
                labels=assigned_labels,
                ood_labels=assigned_ood_labels)
        ...
        assigned_ood_labels = assigned_gt_inds.new_full((num_bboxes, ), 0, dtype=torch.int8)
        pos_inds = torch.nonzero(
            assigned_gt_inds > 0, as_tuple=False).squeeze()
        if pos_inds.numel() > 0:
            assigned_labels[pos_inds] = gt_labels[assigned_gt_inds[pos_inds] -
                                                  1]
            assigned_ood_labels[pos_inds] = gt_ood_labels[assigned_gt_inds[pos_inds] -
                                                  1]
